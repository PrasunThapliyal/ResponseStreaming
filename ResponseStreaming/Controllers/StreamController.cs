...
namespace ResponseStreaming.Controllers
{
    ...
    public class StreamController : ControllerBase
    {
        ...
        /// 
        /// Apparently, a normal multipart/form-data upload using <form> or fetch(FormData) does not support this.
        /// You need:
        /// HTTP/2
        /// Streaming request body (not multipart/form-data)
        /// Streaming response body (flush after each write)
        /// Browser Fetch API with ReadableStream for request body and response body
        /// ASP.NET Core streaming request and response handling
        /// 
        /// </summary>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// None of v1, v2, v3 have worked
        /// It seems full duplex isn't yet supported by browsers, so there's no way to do it in a single API
        /// </summary>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        [HttpPost("api/v3/streamResponse")]
        public async Task<IActionResult> StreamResponseV3(CancellationToken cancellationToken)
        {
            _logger.LogInformation($"StreamResponseV3: request received ..");
            ...
            await using ((streamWriter = new StreamWriter(Response.Body)).ConfigureAwait(false))
            {
                ...
                while ((bytesRead = await Request.Body.ReadAsync(readBuffer)) > 0)
                {
                    ...
                    var uploadProgress = $"Downloaded {bytesReceived} of {totalBytes} \r\n";
                    //byte[] bytes = System.Text.Encoding.UTF8.GetBytes(uploadProgress);

                    _logger.LogInformation($"Upload progress to UI: {uploadProgress}");

                    await streamWriter.WriteAsync(uploadProgress).ConfigureAwait(false);
                    //await streamWriter.WriteLineAsync(uploadProgress).ConfigureAwait(false);
                    await streamWriter.FlushAsync().ConfigureAwait(false);
                }
